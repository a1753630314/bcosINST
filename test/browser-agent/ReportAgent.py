...
HOST_IP = socket.gethostname(); #本机器的外网IP，仅作为浏览器端区分是哪台机器上报的数据
BROWSER_SERVER_IP = "192.168.50.118" #上报server端的IP
BROWSER_SERVER_PORT = "8888" #上报server端的端口

node0 = [HOST_IP, "/bcos-data/node/log.conf", 8545,"/bcos-data/node/log"] #node的名字, log.conf的路径, RPC端口号, node的log目录(可选)
